...
print(f"Temporale Konzentration: 42.8% in WWI-Ära (1906-1918)")
print(f"Theoretisches Maximum: ~364 Tiles bei min_d=40km")
print(f"Aktuell selektiert: 40 (11% des Potentials)")
print()
...
for n_samp, gamma, min_d in itertools.product(n_samples_vals, gamma_temporal_vals, min_distance_vals):
    run_num += 1
    alpha = 1.0 - beta_spatial - gamma
    
    print(f"[{run_num:2d}/{total_runs}] n={n_samp}, γ={gamma:.2f}, min_d={min_d:.0f}km ", end='', flush=True)
    ...
    print(f"→ n={n_sel:2d}, clusters={clusters_covered}/8, temp_std={temporal_std:5.2f}, "
...
print(f"  → n={int(best_temporal['n_samples_target'])}, γ={best_temporal['gamma_temporal']:.2f}, "
      f"min_d={best_temporal['min_distance_km']:.0f}km")
print(f"  → Temporal: STD={best_temporal['temporal_std']:.2f}, Range={best_temporal['temporal_range']:.0f} Jahre")
print(f"  → Spatial: Mean={best_temporal['mean_pairwise_km']:.1f}km, Min={best_temporal['min_pairwise_km']:.1f}km")
print()
...
print(f"  → n={int(best_temporal_std['n_samples_target'])}, γ={best_temporal_std['gamma_temporal']:.2f}, "
...
print(f"  → γ={best_coverage['gamma_temporal']:.2f}, min_d={best_coverage['min_distance_km']:.0f}km")
print(f"  → Temporal: STD={best_coverage['temporal_std']:.2f}, WWI={best_coverage['wwi_fraction']*100:.1f}%")
print(f"  → Spatial: Mean={best_coverage['mean_pairwise_km']:.1f}km")
print()

# Balanced (composite score)
df['temp_diversity_score'] = (df['temporal_std'] - df['temporal_std'].min()) / (df['temporal_std'].max() - df['temporal_std'].min())
df['wwi_score'] = 1 - df['wwi_fraction']  # Lower WWI = better
df['coverage_score'] = (df['n_selected'] - df['n_selected'].min()) / (df['n_selected'].max() - df['n_selected'].min())
df['composite_score'] = 0.4 * df['temp_diversity_score'] + 0.3 * df['wwi_score'] + 0.3 * df['coverage_score']

best_balanced = df.loc[df['composite_score'].idxmax()]
print(f"Beste Balance (Composite Score={best_balanced['composite_score']:.3f}):")
print(f"  → n={int(best_balanced['n_samples_target'])}, γ={best_balanced['gamma_temporal']:.2f}, "
      f"min_d={best_balanced['min_distance_km']:.0f}km")
print(f"  → Selected: {int(best_balanced['n_selected'])}, WWI={best_balanced['wwi_fraction']*100:.1f}%")
print(f"  → Temporal: STD={best_balanced['temporal_std']:.2f}, Range={best_balanced['temporal_range']:.0f} Jahre")
...
for gamma in sorted(df['gamma_temporal'].unique()):
    ...
    print(f"  γ={gamma:.2f}: WWI={mean_wwi*100:4.1f}% (avg), Temp_STD={mean_std:5.2f} (avg)")
...
for n in sorted(df['n_samples_target'].unique()):
    ...
    print(f"  n={n:2d}: Success={success_rate*100:5.1f}% (avg {subset['n_selected'].mean():.1f} selected), "
...
for min_d in sorted(df['min_distance_km'].unique()):
    ...
    print(f"  {min_d:.0f}km: Avg_Selected={mean_n:.1f}, Avg_Min_Dist={mean_min_dist:.1f}km")
