...
def main():
    parser = argparse.ArgumentParser(
        description="Extract best selection and prepare sampler comparison"
    )
    parser.add_argument(
        "--run-id", required=True, help="Run ID (e.g., 20260116_T214724_adaptive_full)"
    )
    parser.add_argument(
        "--wait", action="store_true", help="Wait for run to complete before extracting"
    )
    parser.add_argument(
        "--output",
        default="outputs/kdr100_best_selection_info.json",
