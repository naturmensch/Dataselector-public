...
print(f"Fixed: min_distance_km=40.0, alpha_visual varies to maintain sum=1.0\n")
...
for gamma in gamma_values:
    ...
    print(f"γ={gamma:.2f} (α={alpha:.2f}, β={beta:.2f}): n={n_sel:2d}, "
...
print("ANALYSIS")
print('=' * 80)
print(f"Correlation gamma_temporal ↔ temporal_std: r={df['gamma_temporal'].corr(df['temporal_std']):.3f}")
print(f"Temporal STD range: {df['temporal_std'].min():.2f} - {df['temporal_std'].max():.2f} (Δ={df['temporal_std'].max()-df['temporal_std'].min():.2f})")
print(f"Temporal range: {df['temporal_range'].min():.0f} - {df['temporal_range'].max():.0f} (Δ={df['temporal_range'].max()-df['temporal_range'].min():.0f})")

if abs(df['gamma_temporal'].corr(df['temporal_std'])) > 0.5:
    print(f"\n✅ SUCCESS: Temporal weight hat signifikanten Effekt!")
    print(f"   Multi-Criteria Methode funktioniert wie erwartet.")
else:
    print(f"\n⚠️  Temporal weight zeigt schwachen/keinen Effekt")
    print(f"   Mögliche Ursache: Spatial constraint zu dominant")
