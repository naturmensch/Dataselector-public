...
This script orchestrates all pipeline phases (Phase 0-5) as a thin wrapper around
modularized phase functions in src.pipeline_phases. Each phase is independent
and can be tested/run separately.

Phase Structure:
- Phase 0: Pre-flight (convergence validation from 10-seed Hamburg data)
- Phase 1: XXL Hamburg Run (440 trials, CMA-ES, Seed 42, 673 candidates = 100% dataset)
- Phase 2: Reproducibility Validation (Seeds 43, 44)
- Phase 3: Final Statistics & Report Generation
- Phase 4: Thesis Summary
- Phase 5: Bootstrap Uncertainty Quantification (500 resamples)

Scientific Justification:
- 440 trials = 5× convergence baseline (99% achieved at median ~88 trials from 10-seed Hamburg CMA-ES validation)
...
All phase functions are modularized in src.pipeline_phases for independent testing
and reusability across different orchestration contexts.
"""
...
# Import modularized phase functions
...
# All modularized phase functions are imported from src.pipeline_phases above
# log() and run_cmd_with_retry() are also imported
...
def phase_5_bootstrap(run_dir: Path | None = None, n_boot: int = 500) -> bool:
    """Phase 5: Bootstrap Uncertainty Quantification.
    
    Performs 500 bootstrap resamples to assess stability and generate confidence intervals
    for the final selection (Jaccard index, cluster coverage, temporal diversity).
    
    Args:
        run_dir: Optional specific run directory. If None, auto-detects XXL Hamburg run.
        n_boot: Number of bootstrap resamples (default 500 for thesis-grade robustness).
    
    Returns:
        True on success, False on error.
    """
    log("=" * 70, "PHASE")
    log(f"PHASE 5: BOOTSTRAP UNCERTAINTY QUANTIFICATION ({n_boot} resamples)", "PHASE")
    log("=" * 70, "PHASE")
    
    # Auto-detect XXL Hamburg run if not provided
    target_run_dir = run_dir
    if target_run_dir is None:
        xxl_runs = _find_xxl_runs(ROOT)
        if xxl_runs:
            target_run_dir = xxl_runs[-1]  # Latest run
            log(f"Auto-detected XXL run: {target_run_dir.name}", "PHASE")
        else:
            log("No XXL Hamburg run found for bootstrap", "ERROR")
            return False
    
    if not target_run_dir.exists():
        log(f"Run directory not found: {target_run_dir}", "ERROR")
        return False
    
    # Call bootstrap_final_selection.py as subprocess
    cmd = (
        f"cd {ROOT} && "
        f"{sys.executable} scripts/bootstrap_final_selection.py "
        f"--run-dir {target_run_dir} "
        f"--n-boot {n_boot}"
    )
    
    ret = run_cmd_with_retry(cmd, retries=1, delay=5)
    if ret != 0:
        log(f"❌ Phase 5 Bootstrap failed (exit code {ret})", "ERROR")
        return False
    
    # Verify bootstrap outputs exist
    bootstrap_full = target_run_dir / "results" / "bootstrap_final_selection_full.csv"
    bootstrap_summary = target_run_dir / "results" / "bootstrap_final_selection_summary.csv"
    
    if bootstrap_full.exists() and bootstrap_summary.exists():
        log(f"✅ Bootstrap complete: {bootstrap_full.name}, {bootstrap_summary.name}", "PHASE")
        log("Phase 5 COMPLETE", "INFO")
        return True
    else:
        log("Bootstrap output files not found after execution", "ERROR")
        return False
...
def main() -> int:
    ...
    # Phase 5: Bootstrap Uncertainty Quantification
    if not phase_5_bootstrap(n_boot=500):
        log("Pipeline aborted at Phase 5", "ERROR")
        return 1
    ...
    log("=" * 70)
    log(f"✅ COMPLETE THESIS PIPELINE FINISHED in {hours:.1f} hours", "SUCCESS")
    log("=" * 70)
    ...
    log("  - runs/thesis_xxl_hamburg_final/results/", "SUCCESS")
    log("    └─ bootstrap_final_selection_summary.csv (CIs & stability metrics)", "SUCCESS")
    log("=" * 70)
