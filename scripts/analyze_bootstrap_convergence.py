...
def analyze_convergence(
    ...
    ax1.set_title(
        "Bootstrap Konvergenz: CI-Breite vs. n_boot", fontsize=14, fontweight="bold"
    )
    ax1.set_xscale("log")
    ...
    relative_change = [
        abs(means[i] - means[i - 1]) / means[i - 1] * 100 if i > 0 else 0
    ...
    ax2.set_title(
        "Bootstrap Konvergenz: Relative Änderung", fontsize=14, fontweight="bold"
    )
    ax2.set_xscale("log")
    ...
    if conv_indices:
        ...
        print("   (< 5% Änderung gegenüber vorherigem Wert)")
        ...
        print("⚠️  Keine Konvergenz erreicht. Höhere n_boot Werte testen!")
    ...
    if 200 in n_boot_values:
        ...
        if cv < 5:
            print("   ✅ Sehr stabil (CV < 5%)")
        elif cv < 10:
            print("   ✅ Stabil (CV < 10%)")
        else:
            print("   ⚠️  Hohe Variabilität (CV > 10%), mehr Resamples erwägen")
...
def main():
    ...
    parser.add_argument(
        "--data-file", type=Path, help="CSV-Datei mit Bootstrap-Daten (optional)"
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
    ...
    if args.data_file and args.data_file.exists():
        ...
        if data is None:
            ...
            print(
                f"Synthetische Daten: N={len(data)}, mean={data.mean():.2f}, std={data.std():.2f}"
            )
