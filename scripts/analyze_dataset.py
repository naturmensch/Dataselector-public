...
print("→ Bei min_distance=40km → Sperrkreis ~5,027 km²/tile")
print(f"→ Theoretisches Maximum: ~{int(area_approx/5027)} selectable tiles")
print("→ Tatsächlich selektiert: 40 tiles (Multi-Criteria)\n")
...
if len(df) > unique_sheets:
    ...
    sample_data = df[df["SheetNumber"] == sample_sheet][
        ["SheetNumber", "year", "N", "left"]
    ].sort_values("year")
