...
report.append(
    f"| Mean Value | {hamburg['mean_value']:.6f} | {kdr100['mean_value']:.6f} | {abs(hamburg['mean_value'] - kdr100['mean_value']):.6f} |"
)
report.append(
    f"| Std Dev | {hamburg['std_value']:.6f} | {kdr100['std_value']:.6f} | {abs(hamburg['std_value'] - kdr100['std_value']):.6f} |"
)
report.append(
    f"| 99% Conv. Trial | {int(hamburg['convergence_trial'])} | {int(kdr100['convergence_trial'])} | {abs(int(hamburg['convergence_trial']) - int(kdr100['convergence_trial']))} |"
)
report.append("")
...
report.append("")
report.append(
    f"**Hamburg**: Mean {hamburg['boot_mean']:.6f} with 95% CI [{hamburg['boot_ci_lo']:.6f}, {hamburg['boot_ci_hi']:.6f}]"
)
report.append(
    f"**KDR100**: Mean {kdr100['boot_mean']:.6f} with 95% CI [{kdr100['boot_ci_lo']:.6f}, {kdr100['boot_ci_hi']:.6f}]"
)
report.append("")
report.append(
    "Bootstrap validation confirms robustness of the selected parameters across resampled candidate subsets."
)
report.append("")
...
report.append("")
report.append(
    "1. **Best configuration selected**: CMA-ES with parameters from Hamburg run"
)
...
report.append("")
report.append(
    "2. **Convergence analysis**: CMA-ES converges to 99% best value by ~{:d} trials ({:.1%}) on both datasets".format(
...
report.append("")
report.append(
    "3. **Generalization**: Minimal performance degradation when scaling from Hamburg → KDR100 dataset"
)
report.append(
    f"   - Delta: {abs(hamburg['best_value'] - kdr100['best_value']):.6f} ({abs(hamburg['best_value'] - kdr100['best_value']) / hamburg['best_value'] * 100:.2f}%)"
)
report.append("")
...
report.append(
    f"- Trials CSV: `{hamburg_run}/results/trials.csv` ({hamburg['n_trials']} rows)"
)
report.append(
    f"- Trials CSV: `{kdr100_run}/results/trials.csv` ({kdr100['n_trials']} rows)"
)
report.append("- Bootstrap results: `*/results/bootstrap_results_summary.csv`")
report.append("- Convergence plots: `*/reports/convergence_*.png`")
report.append("")

report.append("---")
report.append(
    f"*Report generated by thesis pipeline on {datetime.utcnow().strftime('%Y-%m-%d %H:%M:%S')} UTC*"
)
