...
for sec in sections:
    ...
    def get_field(field):
        ...
        return ""
    data['imports'] = [s.strip() for s in get_field('Imports (top-level modules)').split(',')] if get_field('Imports (top-level modules)') else []
    data['functions'] = [s.strip() for s in get_field('Top-level functions').split(',')] if get_field('Top-level functions') and '*(none)*' not in get_field('Top-level functions') else []
    data['classes'] = [s.strip() for s in get_field('Top-level classes').split(',')] if get_field('Top-level classes') and '*(none)*' not in get_field('Top-level classes') else []
    data['has_main'] = True if 'Yes' in get_field('Has __main__ guard') else False
    sc = get_field('Subprocess-like calls detected')
    data['subprocess_calls'] = [s.strip() for s in sc.split(',')] if sc and '*(none)*' not in sc else []
    ...
    if category == 'script':
        ...
        if name.startswith('check_') or name.startswith('verify_') or 'diagnose' in tokens:
...
for e in entries:
    ...
    if 'optuna' in n.lower():
        ...
        key = 'docs_link'
    elif 'generate_report' in n.lower() or 'generate' in n.lower() or 'report' in n.lower():
        key = 'reports'
    elif n.lower().startswith('check_') or n.lower().startswith('verify_') or 'diagnose' in n.lower():
        key = 'checks'
    elif e['deprecated']:
        key = 'deprecated'
    if key:
...
for e in entries:
    ...
    if e['category'] == 'script' and (not e['functions'] or len(e['functions']) <= 1) and (not e['has_main']):
...
for k, v in groups.items():
    ...
    if k == 'optuna':
        lines.append("**Suggestion:** Consolidate to a single `optuna` CLI subcommand that supports import/export, autoscale, analysis. Move helper scripts into `dataselector.workflows.optuna` as subcommands.\n")
    elif k == 'compare':
        lines.append("**Suggestion:** Merge `compare_samplers*` variants into a parametrized comparison runner that accepts dataset & seeding strategies as arguments. Keep legacy scripts as archived examples.\n")
    elif k == 'bootstrap':
        lines.append("**Suggestion:** Consolidate bootstrap generation/plotting into a `bootstrap` module with small CLI wrappers.\n")
    elif k == 'xxl':
        lines.append("**Suggestion:** Parameterize XXL-run scripts or unify into a single XXL orchestration CLI; keep one example runner per experiment profile.\n")
    elif k == 'docs_link':
        lines.append("**Suggestion:** Merge docs link fixers into one robust maintenance tool with subcommands (patch, autofix, show patterns).\n")
    elif k == 'reports':
        lines.append("**Suggestion:** Unify report generation and templating to reduce duplicate code; expose as `dataselector.workflows.generate_reports`.\n")
    elif k == 'checks':
        lines.append("**Suggestion:** Move environment and diagnostic checks into `tools/` and add a single CLI entry.\n")
    elif k == 'deprecated':
        lines.append("**Suggestion:** Archive or remove scripts in `deprecated/` after ensuring no active use.\n")
    lines.append('\n')
    # list up to 40 files
