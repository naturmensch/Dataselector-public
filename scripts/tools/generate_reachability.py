...
# Patterns
PY_SCRIPT_PAT = re.compile(r"\bpython\b[^\n]*?\b(?:\.\/)?scripts\/([A-Za-z0-9_\-]+)\.py\b")
EXEC_IN_ENV_PAT = re.compile(r"exec_in_env\.sh[^\n]*?\bpython\b[^\n]*?scripts\/([A-Za-z0-9_\-]+)\.py\b")
    ...
    for fp in sorted(root.rglob('*')):
            ...
            if tree is not None:
                    ...
                    for node2 in ast.walk(tree):
                        if isinstance(node2, ast.Assign):
                            for t in node2.targets:
                                if isinstance(t, ast.Name) and t.id not in var_to_script:
                                    for n2 in ast.walk(node2.value):
                                        if isinstance(n2, ast.Name) and n2.id in var_to_script:
                            ...
                            if isinstance(part.value, ast.Name):
                                ...
                                for n2 in ast.walk(part.value):
                                    if isinstance(n2, ast.Name) and n2.id in var_to_script:
                                        ...
                                        break
                    for sc in found:
                        edges[fp.name][sc].append((fp.as_posix(), getattr(js, 'lineno', 0), 'AST-joinedstr'))
                ...
                for node in ast.walk(tree):
                    if isinstance(node, ast.Call):
                        ...
                        for sc in found:
                            edges[fp.name][sc].append((fp.as_posix(), getattr(node, 'lineno', 0), f'AST-call: {ast.unparse(node.func) if hasattr(ast, "unparse") else "call"}'))
        ...
        for i, line in enumerate(text.splitlines(), start=1):
            ...
            for m in SCRIPT_FILE_REF_PAT.finditer(line):
                ...
                if 'docs' in fp.parts or fp.name.lower().startswith('readme') or 'README' in fp.name:
...
with open(csv_out, 'w', newline='') as fh:
    writer = csv.DictWriter(fh, fieldnames=['script','reachable','shortest_path','evidence_sample','num_callers_found'])
    writer.writeheader()
    for r in rows:
        writer.writerow(r)
...
with open(report, 'w') as fh:
    ...
    fh.write('\n')
    fh.write('## Notes\n')
    fh.write('- This is a static, best-effort analysis based on lexical matches in scripts, tests, docs and README.\n')
    fh.write('- Some paths depend on runtime flags, environment wrappers, or optional imports and might be missed.\n')
