...
def profile_mode(mode_name: str, selector: DiversitySelector, features: np.ndarray, metadata: pd.DataFrame, **kwargs):
...
def main():
    ...
    selector_legacy.select(features, metadata, spatial_constraint=True, min_distance_km=50.0)
    results['legacy'] = profile_mode('legacy', selector_legacy, features, metadata, spatial_constraint=True, min_distance_km=50.0)

    # CONSTRAINT-INTEGRATED mode
    selector_constraint = DiversitySelector(n_samples=34, use_multi_criteria=False, use_constraint_integration=True)
    print('Warm-up (constraint_integrated) to allow JIT compilation...')
    selector_constraint.select(features, metadata, spatial_constraint=True, min_distance_km=50.0)
    results['constraint_integrated'] = profile_mode('constraint_integrated', selector_constraint, features, metadata, spatial_constraint=True, min_distance_km=50.0)
    ...
    if 'year' in metadata.columns:
        ...
        selector_multi.select(features, metadata, alpha_visual=0.7, beta_spatial=0.15, gamma_temporal=0.15)
        results['multi_criteria'] = profile_mode('multi_criteria', selector_multi, features, metadata, alpha_visual=0.7, beta_spatial=0.15, gamma_temporal=0.15)
