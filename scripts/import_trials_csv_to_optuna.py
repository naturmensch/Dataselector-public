...
def main():
    ...
    for p in param_cols:
        ...
        if pd.api.types.is_integer_dtype(col) or all(float(x).is_integer() for x in col):
            lo = int(col.min())
            hi = int(col.max())
            if lo == hi:
                hi = lo + 10
            distributions[p] = optuna.distributions.IntDistribution(low=lo, high=max(hi, lo + 1))
        else:
            lo = float(col.min())
            hi = float(col.max())
            if lo == hi:
    ...
    for i, row in df.iterrows():
        ...
        trial = create_trial(state=state, value=value, params=params, distributions={k: distributions[k] for k in params.keys() if k in distributions})
