...
def main():
    ...
    parser.add_argument(
        "--delete-outputs", action="store_true", help="Delete known output directories"
    )
    parser.add_argument(
        "--delete-venvs",
        action="store_true",
        help="Delete local virtual environment dirs (.venv, venv)",
    )
    parser.add_argument(
        "--archive",
        nargs=2,
        metavar=("SRC", "ARCHIVE"),
        help="Archive SRC to ARCHIVE (tar.gz)",
    )
    parser.add_argument(
        "--protect", action="append", help="Add additional protected path (repeatable)"
    )
