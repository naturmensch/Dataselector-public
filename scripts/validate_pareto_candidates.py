...
def validate(pareto_csv: str, min_distances=[25,35,50], seeds=[42, 43, 44, 45, 46], n_samples: int = 673, output_dir: str = None):
    ...
    for _, row in pareto.iterrows():
        ...
        for min_d in min_distances:
            for seed in seeds:
                run_i += 1
                print(f"Run {run_i}/{total}: α={alpha}, β={beta}, γ={gamma}, min_dist={min_d}, seed={seed}")
                t0 = time.time()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--pareto', type=str, default=str(ROOT / 'outputs' / 'tuning_weights' / 'pareto' / 'pareto_solutions.csv'))
    ...
    validate(pareto_csv=args.pareto, min_distances=args.min_dist, seeds=args.seeds, n_samples=args.n_samples)
