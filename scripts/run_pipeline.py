...
def should_run_tuning(tune_flag: bool, force: bool, ttl_days: int, csv_meta: Path, out_dir: Path) -> bool:
...
def main():
    ...
    parser.add_argument('--tune', action='store_true', help='Run parameter tuning before main pipeline (default: skip)')
    parser.add_argument('--force-tune', action='store_true', help='Force tuning even if cached results exist')
    parser.add_argument('--tune-ttl', type=int, default=7, help='Cache validity in days (default: 7)')
    parser.add_argument('--interactive', action='store_true', help='Ask for confirmation before expensive ops')
    ...
    run_tune = should_run_tuning(args.tune, args.force_tune, args.tune_ttl, CSV_META, OUT_DIR)

    if args.tune and args.interactive:
        resp = input(f"Tuning requested and may take long, proceed? [y/N] ")
    ...
    if run_tune:
        ...
        runner.run_weight_sweep(csv_meta=str(CSV_META), n_samples=673, weight_combinations=None)
    else:
        print("Skipping tuning (cache valid or not requested).")

    # After tuning (or skipping), run main pipeline
    pipeline = KDR100SelectionPipeline(config_path=str(ROOT / 'config' / 'pipeline_config.yaml'))
    pipeline.run()
