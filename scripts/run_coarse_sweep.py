...
print(f"Grid-Größe: {len(weight_combos)} Gewichtungen x {len(min_distances)} Distanzen = {len(weight_combos)*len(min_distances)} Runs")
...
parser.add_argument('--min-distances', type=str, default=None, help='Comma-separated min distances to test')
...
print(f"Feasible-only results saved: {OUTPUT_DIR / 'coarse_sweep_results_feasible.csv'}")
