...
def main():
    ...
    if '--n-samples-min' not in getattr(parser, '_option_string_actions', {}):
        parser.add_argument('--n-samples-min', type=int, default=None, help='If provided together with --n-samples-max, uses a range for n_samples optimization')
    if '--n-samples-max' not in getattr(parser, '_option_string_actions', {}):
        parser.add_argument('--n-samples-max', type=int, default=None, help='If provided together with --n-samples-min, uses a range for n_samples optimization')
    if '--continue-on-analysis-failure' not in getattr(parser, '_option_string_actions', {}):
        parser.add_argument('--continue-on-analysis-failure', action='store_true', help='Do not abort pipeline if Optuna or analysis fails (keep legacy behavior)')
    ...
    args, _ = parser.parse_known_args()  # ignore unknown pytest args when imported during tests
        ...
        try:
            ...
            if not has_optuna:
                ...
                if args.n_samples is not None:
                    ...
                    try:
                        cfg_n = em.manifest.get('config', {}).get('selection', {}).get('n_samples') if hasattr(em, 'manifest') else None
                    except Exception:
                        cfg_n = None
                    if cfg_n:
                ...
                print(f'Running Optuna with min_distance range: {opt_lo}-{opt_hi} km (center {center}km); n_samples={chosen_n_samples or "range mode"}')
                ...
                if chosen_n_samples is not None:
                    optuna_cmd = optuna_cmd.replace('--n-candidates', f'--n-samples {int(chosen_n_samples)} --n-candidates')
                elif args.n_samples_min is not None and args.n_samples_max is not None:
                    optuna_cmd = optuna_cmd.replace('--n-candidates', f'--n-samples-min {int(args.n_samples_min)} --n-samples-max {int(args.n_samples_max)} --n-candidates')
                ...
                try:
                    ...
                    if args.continue_on_analysis_failure:
                        print('Continuing despite Optuna failure (--continue-on-analysis-failure set).')
                    else:
                        print('Aborting pipeline due to Optuna failure. Use --continue-on-analysis-failure to override.')
                        raise SystemExit(1)
