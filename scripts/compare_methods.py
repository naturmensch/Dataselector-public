    ...
    for params in parameters:
        ...
        for method in ['legacy', 'constraint_integrated']:
            ...
            selector = DiversitySelector(
                n_samples=34,
                use_constraint_integration=use_integration
            )
            ...
            print(f"  [{method:20s}] n={n_selected:2d}, clusters={clusters_covered}/{config['n_clusters']}, "
...
for config_name in df['config'].unique():
    ...
    for (min_d, tw), group in subset.groupby(['min_distance_km', 'temporal_weight']):
        ...
        print(f"    Legacy:      n={legacy['n_selected']:2d}, temporal_std={legacy['temporal_std']:.2f}, mean_dist={legacy['mean_pairwise_km']:.1f}km")
        print(f"    Integrated:  n={integrated['n_selected']:2d}, temporal_std={integrated['temporal_std']:.2f}, mean_dist={integrated['mean_pairwise_km']:.1f}km")
        ...
        print(f"    Δ:           n={delta_n:+3d}, temporal_std={delta_temporal:+.2f}, mean_dist={delta_spatial:+.1f}km")
