...
def analyze_coordinates(csv_path):
    ...
    duplicates = df[df.duplicated(subset=["ul_x", "ul_y"], keep=False)].sort_values(
        "ul_x"
    )
    if len(duplicates) > 0:
    ...
    if x_span > 1_000_000 and y_span > 1_000_000:
        print(
            f"✓ Coordinates span {x_span/1e6:.1f} × {y_span/1e6:.1f} M meters (reasonable for Germany)"
        )
    else:
        print(
            f"⚠️  Coordinate span too small: {x_span/1e6:.1f} × {y_span/1e6:.1f} M meters"
        )
        return False
    ...
    print(
        f"Width:  min={df['width_m'].min():.0f} m, median={df['width_m'].median():.0f} m, max={df['width_m'].max():.0f} m"
    )
    print(
        f"Height: min={df['height_m'].min():.0f} m, median={df['height_m'].median():.0f} m, max={df['height_m'].max():.0f} m"
    )
    print(
        f"Median tile size: {(df['width_m'].median()/1000):.1f} × {(df['height_m'].median()/1000):.1f} km"
    )
    ...
    print(
        f"✓ Tiles with NN < 10 km: {small_count} ({100*small_count/len(nn_distances):.1f}%)"
    )
    ...
    if median_nn >= mean_nn - 5:
        ...
        issues.append(
            f"⚠️  Median ({median_nn:.1f}) << Mean ({mean_nn:.1f}) - suggests low outliers"
        )
