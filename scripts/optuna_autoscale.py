...
def make_objective(features, metadata, n_samples, min_distance_bounds):
    def objective(trial: optuna.trial.Trial):
        ...
        min_dist = trial.suggest_int('min_distance_km', *min_distance_bounds['min_distance_km'])
...
def run_autoscale(n_trials_per_stage, stages_samples, features, metadata, patience=2, tol=None):
    ...
    for stage_idx, n_samples in enumerate(stages_samples):
        ...
        try:
            ...
            md_vals = [t.user_attrs.get('min_distance_km') for t in stage_trials if 'min_distance_km' in t.user_attrs]
            if md_vals:
                fig, ax = plt.subplots(figsize=(6,4))
                sns.histplot(md_vals, bins=10, kde=False, ax=ax)
                ax.set_title(f'Stage {stage_idx+1} min_distance distribution')
                out = OUT / f'optuna_autoscale_stage{stage_idx+1}_min_distance_{date}.png'
                fig.savefig(out, bbox_inches='tight')
                plt.close(fig)
        ...
        if best_prev is not None:
            ...
            print(f"deltas: dalpha={dalpha:.4f}, dbeta={dbeta:.4f}, dmin={dmin:.2f}, dval={dval:.4f}")

            tol = tol or {'alpha': 0.02, 'beta': 0.02, 'min_distance_km': 1.0, 'value': 1e-3}

            if dalpha < tol['alpha'] and dbeta < tol['beta'] and dmin < tol['min_distance_km']:
                no_change += 1
    ...
    try:
        ...
        lines.append(f"(could not render markdown table; saved CSV summary: {csv_summary.name})")
    ...
    with out_best.open('w') as fh:
        json.dump({'value': best_overall.value, 'params': best_overall.params, 'user_attrs': best_overall.user_attrs}, fh, indent=2)
...
if __name__ == '__main__':
    ...
    features, metadata = load_or_create_data(n=args.n_candidates, dim=args.dim, seed=args.seed)
    ...
    run_autoscale(n_trials_per_stage, stages, features, metadata, patience=args.patience)
