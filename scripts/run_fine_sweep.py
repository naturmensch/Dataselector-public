...
def _default_weight_combinations():
    """Return the default list of (alpha, beta, gamma) weight tuples."""
    import itertools
...
def main(argv: list | None = None) -> int:
    """Run the fine sweep. Intended to be executed as a script or called programmatically."""
    import argparse
    import os
    ...
    weight_combos = _default_weight_combinations()

    print(
        f"Fine grid size: {len(weight_combos)} weight combos x {len(min_distances)} distances = {len(weight_combos)*len(min_distances)} runs"
    )
    ...
    if exp_dir:
            ...
            if n_samples_cfg is None:
                ...
                if expl_cfg_path.exists():
                    ...
                    if n_expl:
                        ...
                        print(
                            f"ℹ️ Verwende n_samples aus exploration config: {n_samples_cfg}"
                        )
        ...
        if n_samples_cfg is None:
            try:
                ...
                print(
                    f"⚠ selection.n_samples nicht gesetzt; verwende adaptives n_samples={n_samples_cfg}"
                )
            except Exception:
                n_samples_cfg = 34
                print(
                    f"⚠ selection.n_samples nicht gesetzt; Fallback auf n_samples={n_samples_cfg}"
                )
