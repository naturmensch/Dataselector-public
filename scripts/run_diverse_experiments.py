...
def run_experiments(n_runs: int = 20, n_samples: int = 100, seed: int = 42):
    ...
    print(f"Loaded {n_candidates} candidates; running {n_runs} experiments (n_samples={n_samples})")
    ...
    for i in range(n_runs):
        ...
        selector = DiversitySelector(n_samples=n_samples, use_multi_criteria=True, use_lazy_greedy=use_lazy, random_state=seed_run)

        t0 = time.time()
        selected = selector.select(features, metadata, spatial_constraint=True, min_distance_km=min_dist, alpha_visual=alpha, beta_spatial=beta, gamma_temporal=gamma)
        duration = time.time() - t0

        n_selected = len(selected)
        diversity = selector._calculate_diversity_score(features[selected]) if n_selected > 0 else 0.0
        spatial_spread = metadata.loc[selected, ['N','left']].std().mean() if n_selected > 0 else 0.0
        ...
        for idx_i in range(n_selected):
            for idx_j in range(idx_i+1, n_selected):
                ...
                d = haversine_distance(metadata.loc[id1,'N'], metadata.loc[id1,'left'], metadata.loc[id2,'N'], metadata.loc[id2,'left'])
                if d < min_dist - 1e-6:
                    violations += 1
        ...
        print(f"Run {i+1}/{n_runs}: selected={n_selected}, violations={violations}, diversity={diversity:.3f}, spread={spatial_spread:.3f}, t={duration:.2f}s")
    ...
    print('Runs with incomplete selection (<n_samples):', (df['n_selected'] < n_samples).sum())
