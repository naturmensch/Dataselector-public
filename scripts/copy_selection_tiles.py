...
def copy_selected(csv_path: Path, outdir: Path, overwrite: bool, dry_run: bool) -> int:
    ...
    with (
        csv_path.open(newline="", encoding="utf-8") as fh_in,
        manifest_path.open("w", newline="", encoding="utf-8") as fh_out,
    ):
        ...
        for r in reader:
            img_rel = (
                r.get("image_path") or r.get("image_filename") or r.get("filename")
            )
            ...
            for src, dst, want_copy in (
                ...
                if dst.exists() and not overwrite:
                    print(
                        f"[INFO] exists, skipping (use --overwrite to replace): {dst}"
                    )
                else:
...
def main():
    ...
    p.add_argument(
        "--csv", required=True, type=Path, help="Selection CSV (exported by pipeline)"
    )
    p.add_argument(
        "--outdir",
        type=Path,
        default=Path("<qgis-data-root>"),
    ...
    p.add_argument(
        "--flat",
        action="store_true",
        help="Do not create a subfolder inside --outdir; copy files directly into outdir",
    )
    args = p.parse_args()
