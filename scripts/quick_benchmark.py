...
def run_benchmark(subset_size: int = 50, stop_on_success: bool = True):
    ...
    if (
        "image_filename" in metadata.columns
        and metadata["image_filename"].notna().any()
    ):
    ...
    if not candidates:
        raise SystemExit(
            "Keine existierenden Bilddateien im Subset gefunden. Prüfe image paths."
        )
    ...
    for cfg in configs:
        ...
        extractor = FeatureExtractor(
            model_name="resnet50", input_size=224, default_crop_size=cfg["crop_size"]
        )
        ...
        try:
            extractor.extract_features_batch(
                warm,
                image_dir,
                batch_size=min(cfg["batch_size"], len(warm)),
        ...
        try:
            ...
            sel = selector.select(
                feats,
                metadata=metadata.iloc[: len(subset)].reset_index(drop=True),
                temporal_weight=0.2,
            ...
            print(
                f"Config success: extraction={t_ex:.2f}s umap={t_umap:.2f}s selection={t_sel:.2f}s total={total:.2f}s"
            )
            res = {
                "config": cfg,
                "extraction_s": t_ex,
            ...
            np.save(
                OUT / f'quick_features_{cfg["crop_size"][0]}_{cfg["batch_size"]}.npy',
                feats,
            )
            ...
            (OUT / "quick_benchmark_summary.json").write_text(
                json.dumps({"best": res, "all": results}, ensure_ascii=False, indent=2)
            )
    ...
    # none succeeded
    (OUT / "quick_benchmark_summary.json").write_text(
        json.dumps({"best": None, "all": results}, ensure_ascii=False, indent=2)
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--subset", type=int, default=50, help="Subset size for quick run"
    )
    parser.add_argument(
        "--no-stop",
        dest="stop",
        action="store_false",
        help="Do not stop on first success",
    )
