...
if __name__ == '__main__':
    ...
    parser.add_argument('--n-trials', type=int, default=500, help='Trials per sampler in comparison')
    parser.add_argument('--datasets', nargs='+', default=['hamburg','kdr100'], help='Datasets to compare on')
    parser.add_argument('--samplers', nargs='+', default=['qmc','tpe','cmaes'])
    parser.add_argument('--sequential', action='store_true', help='Run sequentially')
    parser.add_argument('--n-trials-full', type=int, default=2000, help='Trials for full adaptive runs')
...
=======
        (suite_dir / 'best_sampler_summary.json').write_text(json.dumps({'best': best, 'summary_table': table.to_dict(orient='records')}, indent=2))
>>>>>>> ci/add-smoke-tests
    except Exception as e:
