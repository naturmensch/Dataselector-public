...
# Selection
selector = DiversitySelector(n_samples=n_samples, use_multi_criteria=True, random_state=seed)
print(f"Running final selection: n_samples={n_samples}, α={alpha}, β={beta}, γ={gamma}, min_dist={min_distance_km}")
start = time.time()
...
sel_csv = OUT / f'final_selection_n{n_samples}_a{alpha}_b{beta}_g{gamma}_d{int(min_distance_km)}.csv'
...
metrics.update({'alpha':alpha,'beta':beta,'gamma':gamma,'min_distance_km':min_distance_km,'n_requested':n_samples,'n_selected': len(selected_idx),'duration_s':duration})

# Visualizations
viz = Visualizer(output_dir=str(OUT))
viz.create_summary_report(embeddings_2d=embeddings_2d, cluster_labels=cluster_labels, metadata=metadata, selected_indices=selected_idx, output_prefix=f'final_n{n_samples}')
