...
def summarize_csv_metrics(csv_path: Path) -> dict:
    ...
    try:
        with csv_path.open('r', encoding='utf-8') as fh:
            ...
            keys = ['n_selected','temporal_std','spatial_mean_km','wwi_percent','clusters_covered','total_runs','infeasible_count','infeasible_pct','median_n_selected']
            summary = {}
            for k in keys:
                if k in rows[0]:
                    try:
...
def collect_logs(outdir: Path):
    ...
    for step in ['coarse_sweep','fine_sweep','optuna','bootstrap','final_selection','adaptive_pipeline','tuning_weights']:
        p = outdir / f"{step}.log"
        if p.exists():
            try:
...
def main(argv=None):
    ...
    parser.add_argument('--outdir', required=True, help='Experiment run folder (outputs/experiments/run_...)')
    ...
    optuna_cfg = None
    for candidate in ['pipeline_config.optuna.yaml','optuna_test_config.yaml','pipeline_config.yaml.optuna_bak']:
        p = outdir / candidate
        if p.exists():
            try:
    ...
    summaries = {}
    csv_names = ['coarse_sweep_results.csv','fine_sweep_results.csv','optuna_results.csv','bootstrap_results_summary.csv','feasibility_combined_summary.csv','optuna_convergence_analysis.csv','pareto_solutions.csv']
    ...
    if feasibility_dir.exists():
        ...
        if feas_summary.exists():
            lines.append(f"Combined feasibility summary available: `{feas_summary.name}`\n")
    ...
    if 'optuna_convergence_analysis.csv' in summaries or (outdir.parent / 'optuna_convergence_analysis.csv').exists():
