...
import scripts.bootstrap_pareto_candidates as bpc


def test_bootstrap_main_ensemble_mode(tmp_path, monkeypatch):
    # Create tiny pareto csv
    df = pd.DataFrame({'alpha':[0.6],'beta':[0.2],'gamma':[0.2],'min_distance_km':[28],'n_selected':[10]})
    ...
    monkeypatch.setattr('scripts.bootstrap_pareto_candidates.load_metadata', fake_load_metadata)
    monkeypatch.setattr('scripts.bootstrap_pareto_candidates.load_or_extract_features', fake_load_or_extract_features)
    ...
    monkeypatch.setattr('scripts.bootstrap_pareto_candidates.ClusteringPipeline', DummyClustering)

    out = tmp_path / 'out.csv'

    # Run main in ensemble mode with small training budget
    bpc.main(str(p), n_boot=20, output_csv=str(out), random_seed=1, uq_method='ensemble', n_ensemble_models=2, ensemble_epochs=2)
