...
def test_preselection(tmp_path, test_case, stub_feature_extraction):
    ...
    features = src.io.load_or_extract_features(tmp_path, csv_meta=str("data/new_all_tiles.csv"), cache=True)
    ...
    ds = DiversitySelector(n_samples=test_case["n_samples"], use_multi_criteria=True, random_state=42)
    selected = ds.select(
        features=features,
        metadata=meta,
        alpha_visual=0.7,
