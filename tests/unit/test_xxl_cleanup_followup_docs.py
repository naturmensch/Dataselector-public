...
def test_adr_002_is_marked_superseded():
    text = (ROOT / "docs" / "adr" / "ADR-002-workflow-decision-matrix.md").read_text(
...
def test_legacy_xxl_ops_archive_has_no_broken_relative_links():
    ...
    assert (
        ...
        f"{src.relative_to(ROOT)}: [{text}]({target})" for src, target, text in broken
    )
