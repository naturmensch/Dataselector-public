...
class FakeConnRows:
    ...
    def fetchall(self):
        ...
        if self._last.startswith('select trial_id, number, datetime_start'):
            # return 3 trials
            return [(1, 0, None, None, 'COMPLETE'), (2, 1, None, None, 'COMPLETE'), (3, 2, None, None, 'RUNNING')]
        if self._last.startswith('select trial_id, param_name, param_value from trial_params'):
            return [(1, 'a', '0.1'), (1, 'n_samples', '20'), (2, 'a', '0.2'), (2, 'n_samples', '20')]
        if self._last.startswith('select trial_id, key, value from trial_user_attributes'):
...
def test_reconstruct_falls_back_to_direct_sqlite_when_optuna_missing(monkeypatch, tmp_path):
