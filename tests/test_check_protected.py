from scripts import check_protected as cp
...
def test_default_protected_contains_expected():
    p = cp.get_protected_paths()
    expected = ['data/images', 'data/archive', 'models', 'outputs/final_selection', 'outputs/kdr100_selection']
    for e in expected:
...
def test_offending_files():
    ...
    staged = ['data/images/0001.png', 'README.md', 'src/foo.py', 'outputs/special/report.csv']
