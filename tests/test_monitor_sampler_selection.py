...
def test_monitor_uses_selected_sampler(tmp_path, monkeypatch):
    ...
    if not ('--sampler cmaes' in out or "Using selected sampler from artifact: cmaes" in out):
        # Fallback: check any monitor logs under outputs for the message
        found = False
        for p in (ROOT / 'outputs').glob('**/*.log'):
            try:
                if 'Using selected sampler for Optuna from artifact: cmaes' in p.read_text(errors='ignore') or '--optuna-sampler cmaes' in p.read_text(errors='ignore'):
                    found = True
                    break
            except Exception:
                continue
