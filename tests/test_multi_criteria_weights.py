...
def test_valid_weights_do_not_raise():
    meta = _make_meta()
    # weights sum to 1.0
    m = MultiCriteriaFacilityLocation(n_samples=2, metadata=meta, alpha_visual=0.7, beta_spatial=0.15, gamma_temporal=0.15)
...
def test_invalid_weights_raise_value_error():
    ...
    with pytest.raises(ValueError) as excinfo:
        MultiCriteriaFacilityLocation(n_samples=2, metadata=meta, alpha_visual=0.5, beta_spatial=0.1, gamma_temporal=0.2)
