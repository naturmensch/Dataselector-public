...
def test_run_single_sampler_validates_output(tmp_path, monkeypatch):
    ...
    with patch('subprocess.run') as mock_run, patch('scripts.compare_samplers.time.sleep'):
        ...
        assert compare_samplers.run_single_sampler('qmc', 1, 1, 1, None, 'desc') == str(run_dir)
...
def test_parallel_execution_smoke(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(compare_samplers.multiprocessing, 'get_context', lambda x: mock_context)
    monkeypatch.setattr(compare_samplers.multiprocessing, 'get_all_start_methods', lambda: ['fork'])
    monkeypatch.setattr(compare_samplers, 'ROOT', tmp_path)
