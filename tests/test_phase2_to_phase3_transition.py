...
def _make_fake_xxl_run(
    root: Path, name: str = "20260119_T000000_hamburg_xxl_final"
) -> Path:
...
def test_phase2_to_phase3_transition(tmp_path, monkeypatch):
    ...
    _run_dir = _make_fake_xxl_run(tmp_path)
    ...
    ok2 = xxl.phase_2_reproducibility(
        seeds=[43, 44], n_trials=10, n_candidates=20, pass_params=True, dry_run=False
    )
    assert ok2 is True
    ...
    repro_marker = (
        ...
        / "outputs"
        / "repro_runs"
        / "thesis_hamburg_reproducibility_s43"
        / "completed.marker"
    )
