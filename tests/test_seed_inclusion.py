...
def test_seed_included_by_index(monkeypatch):
    ...
    meta = pd.DataFrame({"longName": ["SEED_001", "other"], "N": [50.0, 51.0], "left": [7.0, 8.0], "year": [1900, 1901]})
    features = np.zeros((2, 64))

    ds = DiversitySelector(n_samples=2, use_multi_criteria=False)
    selected = ds.select(features=features, metadata=meta, spatial_constraint=False, pre_selected=[0])
...
def test_seed_included_by_name(monkeypatch):
    ...
    class FakeMC:
        def __init__(
            ...
            self.preselected_indices = list(preselected_indices) if preselected_indices is not None else []
            self.ranking = []
    ...
    monkeypatch.setitem(sys.modules, "dataselectormulti_criteria_facility_location", fake_mod)
    ...
    selected = ds.select(features=features, metadata=meta, spatial_constraint=False, pre_selected_names=["SEED_001"])
...
def test_seed_included_by_substring_case_insensitive(monkeypatch):
    ...
    meta = pd.DataFrame({"longName": ["KDR_001.png", "other"], "N": [50.0, 51.0], "left": [7.0, 8.0], "year": [1900, 1901]})
    features = np.zeros((2, 64))

    ds = DiversitySelector(n_samples=2, use_multi_criteria=True)
    selected = ds.select(features=features, metadata=meta, spatial_constraint=False, pre_selected_names=["kdr_001"])
