...
def test_full_pipeline_simulation(tmp_dirs, repo_root, inject_src_stub, monkeypatch):
    ...
    np.save(
        outputs / "features.npy", np.zeros((4, 64), dtype=np.float32)
    )  # stale: 4 rows vs 6
    ...
    divsel_mod = load_module_from_path(
        "diversity", repo_root / "dataselector" / "selection" / "diversity_selector.py"
    )
    io_mod = load_module_from_path(
        "io_mod", repo_root / "dataselector" / "data" / "io.py"
    )
    metrics_mod = load_module_from_path(
        "metrics_mod", repo_root / "dataselector" / "analysis" / "metrics.py"
    )
    ...
    monkeypatch.setattr(
        divsel_mod.DiversitySelector,
        "_calculate_diversity_score",
        lambda self, feats: float(np.mean(feats)) if len(feats) > 0 else 0.0,
    )

    # 7) Ensure cache is (re)generated via load_or_extract_features to test cache validation logic
    cache_mod = load_module_from_path(
        "cache_mod", repo_root / "dataselector" / "pipeline" / "cache.py"
    )
...
def test_feature_cache_write_permission_error(tmp_dirs, repo_root, monkeypatch):
    ...
    with pytest.raises(PermissionError, match=r"Disk full \(simulated\)"):
        io_mod.load_or_extract_features(
            out_dir=str(outputs), csv_meta=str(csv_meta), batch_size=1, cache=True
        )
