...
def _run(cmd: List[str], env=None, cwd=REPO_ROOT, check=True):
    ...
    use_wrapper = EXEC_WRAPPER.exists() and os.environ.get("SKIP_EXEC_IN_ENV", "0") != "1"
    final_cmd = []
    if use_wrapper:
...
def data_symlink(tmp_path):
    """Create minimal test subset in tmp_path and symlink data/ to it. Temporarily moves original data/."""
...
def test_e2e_thesis_pipeline(data_symlink):
    ...
    autoscale_reports = list((REPO_ROOT / "outputs").glob("optuna_autoscale_report_*.md"))
    ...
    _run([
        "python",
        "scripts/xxl_KDR146_run_thesis_complete_modern.py",
        "--best-sampler",
        best,
        "--smoke",
    ], env=env)
    ...
    if best_info.exists():
        ...
        assert "selected_tiles" in info_data, "Best selection info missing selected_tiles"
        assert isinstance(info_data["selected_tiles"], list), "selected_tiles not a list"

    # Check logs for key function calls (e.g., ensure imports and functions are triggered)
    log_files = list((REPO_ROOT / "outputs").glob("**/*.log")) + list((REPO_ROOT / "outputs").glob("**/*.txt"))
    all_logs = ""
    ...
    key_groups = [
        ...
        ["XXL THESIS COMPLETE PIPELINE", "XXL THESIS PIPELINE COMPLETE", "✅ XXL THESIS PIPELINE COMPLETE!"],  # From xxl script (allow variants)
    ]
    for group in key_groups:
        assert any(k.lower() in all_logs.lower() for k in group), f"None of indicators {group} found in logs"
    ...
    monitors = list((REPO_ROOT / "outputs" / "runs").glob("**/monitor_reports/monitor_report_*.md"))
...
def test_phase5_dry_run(data_symlink):
    ...
    assert (sim_results / "bootstrap_final_selection_full.csv").exists(), f"Simulated full bootstrap results missing in {sim_results}"
    assert (sim_results / "bootstrap_uq_summary.json").exists(), f"Simulated UQ summary missing in {sim_results}"
    # Ensure Phase 1 created a best_trial.json for the run
    assert (latest / "results" / "best_trial.json").exists(), f"best_trial.json missing in {latest / 'results' }"
...
def test_e2e_with_samplers(data_symlink, sampler):
    ...
    expected_files = ["bootstrap_results_summary.csv", "convergence_baseline.json", "kdr100_best_selection_info.json"]
    for ef in expected_files:
        assert any(ef in art for art in artifacts), f"Expected artifact {ef} not in summary for {sampler}"
...
def test_e2e_with_samplers_smoke(data_symlink, sampler):
    ...
    artifacts = data["artifacts"]
    assert any("bootstrap" in a for a in artifacts), "No bootstrap artifact recorded in finalization summary"
    assert any("convergence_baseline.json" in a for a in artifacts), "convergence_baseline.json not in artifacts"
...
def test_seed_reproducibility(data_symlink):
    ...
    assert len(data1["artifacts"]) == len(data2["artifacts"]), "Reproducibility failed: different artifact counts"
...
def test_monitor_starts_shell_orchestrator(data_symlink, tmp_path):
    ...
    # Run monitor pointing to the shell-based orchestrator
    _run([
        "python",
        "scripts/xxl_full_run_monitor.py",
        "--no-new-session",
    ...
    assert "COMPLETE THESIS PIPELINE" in txt or "COMPLETE THESIS PIPELINE" in txt.upper()
...
def test_monitor_resume_dry_run(data_symlink, tmp_path):
    ...
    trials_csv.write_text("trial_number,value\n0,0.5\n1,0.6\n2,0.7\n")  # Only 3 completed out of 10
    ...
    assert "Planned resume phases" in res.stdout or "Planned resume phases" in res.stderr, "Monitor did not output planned phases"
...
def test_monitor_reconcile_with_db(data_symlink, tmp_path, monkeypatch):
    ...
    assert "Planned resume phases" in res.stdout or "Planned resume phases" in res.stderr, "Monitor did not output planned phases when DB present"
    # Expect that one of the planned phases is 'optuna' (resume)
    assert "optuna" in res.stdout or "optuna" in res.stderr or "reconstructed" in res.stdout or "reconstructed" in res.stderr, "Monitor did not plan optuna resume or reconstruction"
