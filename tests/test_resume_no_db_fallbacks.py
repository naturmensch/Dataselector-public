...
def _make_run_with_trials(tmp_path: Path, name: str = "20260119_T000000_hamburg_xxl_final", n_trials: int = 3):
...
def test_resume_uses_existing_trials_csv(monkeypatch, tmp_path):
    ...
    run_dir = _make_run_with_trials(tmp_path, n_trials=3)
    ...
    assert any(p['name'] in ('reproducibility','finalize','optuna') for p in res.get('phases', []))
