...
def test_resume_executes(monkeypatch, tmp_path):
    ...
    (run_dir / 'manifest.json').write_text(json.dumps({'status': 'attached', 'metadata': {'n_trials': 8}}))
    ...
    assert res['remaining_requested'] == max(0, cfg['n_trials'] - res['completed_before'])
    ...
    assert meta['completed_after'] is not None or meta['ok'] == True
...
def test_staged_resume_after_optuna_complete(monkeypatch, tmp_path):
    ...
    (run_dir / 'manifest.json').write_text(json.dumps({'status': 'attached', 'metadata': {'n_trials': 4}}))
