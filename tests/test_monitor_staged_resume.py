...
def test_db_newer_reconstruct_and_finalize(monkeypatch, tmp_path):
    ...
    # small CSV
    (results / 'trials.csv').write_text('trial_number,state,value\n0,TrialState.COMPLETE,10\n1,TrialState.COMPLETE,20\n')
    # config expects 5
    ...
    def fake_recon(rundir, active_log):
        ...
        txt += '\n'.join(f"{i},TrialState.COMPLETE,{100+i},0,0,0,40,25" for i in range(5))
    ...
    def fake_run_hook(name, cmd_str, base_log_dir, active_log, timeout, retries, env, start_new_session, pass_dry_run):
...
def test_optuna_remaining_resume_and_finalize(monkeypatch, tmp_path):
    ...
    def fake_reconcile(rd, active_log):
        return {'ok': True, 'source': 'trials_csv', 'completed_count': 2, 'best_value': 20.0, 'actions': [], 'reason': None, 'db_path': None, 'attempts': []}
    ...
    assert any(p['name'] == 'optuna' for p in phases) or any(t['name'] == 'optuna' for t in res.get('phases', []))
    assert (tmp_path / 'outputs' / 'THESIS_FINAL_SELECTION_XXL.json').exists()
