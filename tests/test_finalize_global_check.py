...
def _make_run(tmp_path: Path, name: str = 'run_current_hamburg_xxl'):
    ...
    (run_dir / 'config' / 'config_optuna.yaml').write_text(yaml.safe_dump({'n_trials': 2}))
...
def test_resume_schedules_finalize_if_global_final_exists_for_other_run(monkeypatch, tmp_path):
    # Prepare a run
    run_dir = _make_run(tmp_path, name='run_current_hamburg_xxl')
    ...
    (tmp_path / 'outputs' / 'THESIS_FINAL_SELECTION_XXL.json').parent.mkdir(parents=True, exist_ok=True)
    (tmp_path / 'outputs' / 'THESIS_FINAL_SELECTION_XXL.json').write_text(json.dumps(global_sel))

    # Monkeypatch run_hook to noop
    monkeypatch.setattr(monitor, 'run_hook', lambda **kw: {'success': True})

    res = monitor._resume_run('last', tmp_path / 'monitor.log', force=True, dry_run=False)
    assert res.get('ok') is True
    assert any(p['name'] == 'finalize' for p in res.get('phases', [])), 'Finalize should be scheduled when global final is for another run'
...
def test_resume_skips_finalize_if_global_final_matches_run(monkeypatch, tmp_path):
    ...
    res = monitor._resume_run('last', tmp_path / 'monitor.log', force=True, dry_run=False)
    assert res.get('ok') is True
    assert not any(p['name'] == 'finalize' for p in res.get('phases', [])), 'Finalize should be skipped when global final matches this run'