...
def _make_pareto_csv(tmp_path):
    ...
    pd.DataFrame({'N':[50,51,52],'left':[10,11,12],'year':[1900,1914,1918],'image_path':['a','b','c']}).to_csv(out / 'metadata.csv', index=False)
...
def test_validate_small(tmp_path, monkeypatch):
    ...
    df = validate(pareto, min_distances=[10], seeds=[1,2], n_samples=2, output_dir=outdir)
