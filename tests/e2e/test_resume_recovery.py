...
def test_resume_recovery_checkpoint_creation(tmp_workspace: Path, sample_csv: Path, run_dataselector_cli):
...
@pytest.mark.error
@pytest.mark.recovery
def test_resume_graceful_missing_checkpoint(tmp_workspace: Path, sample_csv: Path, run_dataselector_cli):
    ...
    if result.returncode != 0:
        ...
        assert "checkpoint" in stderr.lower() or "resume" in stderr.lower(), \
