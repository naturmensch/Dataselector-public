...
def test_autoscale_smoke(tmp_workspace: Path, sample_csv: Path, run_dataselector_cli):
    ...
    cmd = [
        "autoscale",
        "--csv", str(sample_csv),
        "--stages", "3", "5",  # Very small stages for speed
        "--n-trials", "2", "3",  # Match stage count
...
@pytest.mark.autoscale
def test_autoscale_output_structure(tmp_workspace: Path, sample_csv: Path, run_dataselector_cli):
    ...
    cmd = [
        ...
        "--stages", "3", "5",
        "--output-dir", str(output_dir),
        "--n-candidates", "20",  # Small for speed
    ]
...
@pytest.mark.autoscale
def test_autoscale_multi_stage(tmp_workspace: Path, sample_csv: Path, run_dataselector_cli):
    ...
    cmd = [
        "autoscale",
        "--csv", str(sample_csv),
        "--stages", "10", "20", "10",  # Abbreviated for testing
        "--output-dir", str(output_dir),
        "--n-candidates", "30",
...
def test_autoscale_missing_csv(tmp_workspace: Path, run_dataselector_cli):
    ...
    cmd = [
        "autoscale",
        "--csv", str(tmp_workspace / "nonexistent.csv"),
        "--output-dir", str(output_dir),
    ]
    ...
    assert "not found" in stderr.lower() or "error" in stderr.lower() or "no such" in stderr.lower(), \
