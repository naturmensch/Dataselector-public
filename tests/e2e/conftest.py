...
def pytest_configure(config):
    ...
    config.addinivalue_line("markers", "smoke: fast sanity check (runs in <5 min)")
    config.addinivalue_line(
        "markers",
        "integration: integration test combining multiple components (5-15 min)",
    )
    config.addinivalue_line(
        "markers", "workflow: complete end-to-end workflow test (10-30+ min)"
    )
    config.addinivalue_line(
        "markers", "error: error handling / graceful degradation test"
    )
    ...
    for item in items:
        ...
        if "skip_if_no_gis" in item.fixturenames:
            try:
                ...
                item.add_marker(pytest.mark.skip(reason="geopandas not available"))
...
def dataselector_cli_available() -> bool:
    ...
    try:
        ...
        result = subprocess.run(
            ["dataselector", "--help"], capture_output=True, timeout=5
        )
