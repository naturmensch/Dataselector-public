...
def test_build_tiles_from_real_images(tmp_workspace: Path, run_dataselector_cli):
    ...
    with open(output_csv) as f:
        header = f.readline()
        assert "id" in header, "CSV missing 'id' column"
...
@pytest.mark.geo
@pytest.mark.real_tiles
def test_real_tiles_csv_alignment(real_tiles_csv: Path, tmp_workspace: Path, run_dataselector_cli):
