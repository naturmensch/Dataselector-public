...
@pytest.mark.uq
def test_uq_reproducibility_same_seed(tmp_workspace: Path, sample_csv: Path, run_dataselector_cli):
    """Run pipeline twice with same seed, verify identical UQ estimates.
    
    Validates reproducibility of uncertainty quantification.
    """
    output_dirs = [tmp_workspace / "outputs" / "run1", tmp_workspace / "outputs" / "run2"]
    ...
    if len(results) == 2 and "quantiles" in results[0]:
        assert results[0]["quantiles"] == results[1]["quantiles"], \
            "UQ estimates not reproducible with same seed"


@pytest.mark.integration  
@pytest.mark.uq
def test_uq_different_seeds(tmp_workspace: Path, sample_csv: Path, run_dataselector_cli):
