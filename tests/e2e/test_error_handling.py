...
@pytest.mark.error
def test_autoscale_insufficient_samples(tmp_workspace: Path, sample_csv: Path, run_dataselector_cli):
    ...
    cmd = [
        ...
        "--csv", str(sample_csv),
        "--output-dir", str(output_dir),
        "--n-samples", "1000",  # More than sample_csv has
    ]
    ...
    if result.returncode != 0:
        ...
        assert any(
            ...
            for word in ["sample", "available", "tile", "exceed"]
        ), f"Error message not helpful: {stderr}"


@pytest.mark.error
def test_invalid_parameter_value(tmp_workspace: Path, sample_csv: Path, run_dataselector_cli):
    ...
    cmd = [
        ...
        "--csv", str(sample_csv),
        "--output-dir", str(output_dir),
        "--n-trials", "invalid",  # Should be int
    ]
...
@pytest.mark.error
def test_output_dir_permission_denied(tmp_workspace: Path, sample_csv: Path, run_dataselector_cli):
