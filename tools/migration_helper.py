...
def extract_argparse_calls(file_path: str) -> list[dict]:
    ...
    for match in matches:
        ...
        if type_match:
            ...
            arg_info['type'] = 'int' if type_name == 'int' else 'str' if type_name == 'str' else type_name
...
def generate_decorator(command_name: str, args_list: list[dict], help_text: str = "") -> str:
    ...
    for arg in args_list:
        ...
        if arg['help']:
            decorator_lines.append(f'            "help": "{arg["help"].replace('"', '\\"')}",')
...
def generate_function_signature(args_list: list[dict]) -> str:
    ...
    for arg in args_list:
        ...
        params.append(f'    {arg["py_name"]}: {type_hint} | None = {default_val}' 
...
def main():
    if len(sys.argv) < 2:
        ...
        print("Example: python migration_helper.py dataselector/workflows/adaptive_pipeline.py adaptive-pipeline")
        sys.exit(1)
    
    file_path = sys.argv[1]
    command_name = sys.argv[2] if len(sys.argv) > 2 else Path(file_path).stem.replace('_', '-')
