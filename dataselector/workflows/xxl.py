...
def read_autoscale_config(out_dir: Path) -> dict:
    ...
    if best_json.exists():
        try:
            ...
            log("INFO", f"Read hyperparams from autoscale: α={config['alpha']:.3f}, β={config['beta']:.3f}, γ={config['gamma']:.3f}")
...
def phase_0_preflight(autoscale_config: dict, best_sampler: str, smoke: bool = False) -> bool:
    ...
    missing = [k for k in ("n_samples", "alpha", "beta", "gamma", "min_distance_km") 
    ...
    if not smoke:
        for s in [43, 44]:
            ...
            repro_args = [
                ...
                "--sampler", best_sampler,
                "--seed", str(s),
                "--exp-name", f"repro_seed{s}_{timestamp}",
            ]
