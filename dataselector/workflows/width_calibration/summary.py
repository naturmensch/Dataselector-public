...
def summarize_width_calibration(
    *,
    ...
    measurements_path = resolve_path(measurements_csv, repo_root_path=repo_root_path, prefer_repo=False).resolve()
    ...
    measurements_df["width_px_num"] = pd.to_numeric(measurements_df["width_px"], errors="coerce")
    ...
    repeat_pairs["abs_diff_px"] = (repeat_pairs["width_px_num"] - repeat_pairs["primary_width_px"]).abs()
    ...
    for class_id in present_classes:
        ...
        summary_rows.append(
            SummaryRow(
                ...
                repeat_median_abs_diff_px=round(float(np.median(repeat_abs)), 6) if repeat_abs.size else np.nan,
                low_evidence_flag=bool(values.size < 5),
                high_variance_flag=bool(iqr(values) > 2.0) if values.size else False,
                low_reliability_flag=bool(float(np.median(repeat_abs)) > 1.0) if repeat_abs.size else False,
